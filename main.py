import signal, struct, time, hid, pynput
...
class nintendo_joycon(object):
    ...
    def __open_device(self):
        try:
            ...
            raise IOError("Joy-Con failed to connect")
    ...
    def __send_data(self, rumble_type=b'\x01', sub_cmd_id=None, sub_cmd_data=None):
        ...
        """
        try:
            self.__device.write(rumble_type + self.global_packet_nbr + self.__rumble_data + sub_cmd_id + sub_cmd_data)        
            ...
            while True:
                ...
                for bit_pos in self.__buttons_status["b_pos"]:
                    ...
                    if tmp_bval in self.__buttons_status["val"]:
                        print(self.__buttons_status["val"][tmp_bval][bit_pos - len(self.__buttons_status["b_pos"])])
        except ServiceExit:
            self.__close_device()
        except struct.error:
            self.__read_device()
